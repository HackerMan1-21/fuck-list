...
import { Actor, AllGenre, Countrys, Director, List, Medias, Sales } from "../../src/list/list";

const app = express();
app.use(express.json());

// リスト持ってくる
app.post("/api/type/list", (req, res) => {
	res.json(listDummy);
});


// ID持ってくる
app.post("/api/type/find ", (req, res) => {
	const id = req.body.id;
	if (id !== null && id !== undefined && !isNaN(id)) {
		const data = listDummy.find((e) => e.id === id);
		if (data) {
			res.json(data);
			return;
		}
...
app.post("/api/people/find", (req, res) => {
	const id = req.body.id;
	if (id !== null && id !== undefined && !isNaN(id)) {
		const data = listDummy.find((e) => e.id === id);
		if (data) {
...
app.post("/api/types/regist", (req, res) => {
...
app.post("/api/people/regist", (req, res) => {
	const name = req.body.name;
	const color = req.body.color;
	console.log({ name, color });
	if (
...
export const mediaDummy: Medias[] = [
	{
		id: 1,
		media: "DVD",
	},
...
export const allGenreDummy: AllGenre[] = [
	{
		id: 1,
		genre: "ホラー",
	},
...
export const directorDummy: Director[] = [
	{
		id: 1,
		name: "ジョシュア・サフディ",
	},
...
export const countrysDummy: Countrys[] = [
	{
		id: [1],
		country: ["アメリカ"],
	},
